    ...
    async start() {
        ...
        if (UI.webData.dockerImageVersion) {
            document.getElementById('noVNC_version_docker_image')
                .innerText = 'Docker 镜像 v' + UI.webData.dockerImageVersion;
            document.getElementById('noVNC_version_docker_image')
                .classList.remove("noVNC_hidden");
        }
    ...
    updateVisualState(state) {
        ...
        switch (state) {
            case 'init':
                break;
            case 'connecting':
                transitionElem.textContent = "连接中...";
            ...
            case 'connected':
                ...
                break;
            case 'disconnecting':
                transitionElem.textContent = "断开连接中...";
                document.documentElement.classList.add("noVNC_disconnecting");
                break;
            case 'disconnected':
                break;
            case 'reconnecting':
                transitionElem.textContent = "重新连接中...";
                document.documentElement.classList.add("noVNC_reconnecting");
                break;
            default:
                Log.Error("Invalid visual state: " + state);
                UI.showStatus("内部错误", 'error');
                return;
        }
 ...
 * ------v------*/
    ...
    connect(event, password) {
        ...
        try {
            ...
            UI.showStatus("连接服务器失败: " + exc, 'error');
            return;
        }
    ...
    connectFinished(e) {
        ...
        if (UI.getSetting('encrypt')) {
            msg = UI.desktopName + " - 已连接（加密）";
        } else {
            msg = UI.desktopName + " - 已连接（未加密）";
        }
    ...
    disconnectFinished(e) {
        ...
        UI.showStatus(UI.desktopName + " - 已断开连接", 'error');
 ...
 * ------v------*/
    ...
    credentials(e) {
        ...
        UI.showStatus("需要输入凭据", "warning", 3000, true);
    },

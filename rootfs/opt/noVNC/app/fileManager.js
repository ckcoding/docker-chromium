...
const FileManager = (function () {
    ...
    function showError(message) {
        if (typeof message === 'object') {
            let errMsg = message.error === undefined ? "未知错误" : message.error;
            ...
            if (message.req !== undefined) {
                switch (message.req.type) {
                    case 'listDir':
                        ...
                        break;
                    case 'rename':
                        errMsg = `重命名操作失败：${errMsg}。`;
                        break;
                    case 'delete':
                        errMsg = `删除操作失败：${errMsg}。`;
                        break;
                    case 'createFolder':
                        errMsg = `创建目录操作失败：${errMsg}。`;
                        break;
                    case 'upload':
                    case 'uploadBlock':
                        errMsg = `上传操作失败：${errMsg}。`;
                        break;
                    case 'download':
                        errMsg = `下载操作失败：${errMsg}。`;
                        break;
                    default:
                        errMsg = `操作失败：${errMsg}。`;
                        break;
    ...
    function confirmRename(targetElem, name, path, isDir) {
        ...
        const popoverTitle = `重命名${isDir ? '目录' : '文件'}`;
        const popoverContent = `
            <form id="popoverForm">
                <div class="mb-3">
                    <label for="popoverUserTextInput" class="form-label">将${isDir ? '目录' : '文件'}重命名为：</label>
                ...
                <div class="d-flex justify-content-end column-gap-2">
                  <button type="button" class="btn btn-secondary" id="popoverCancelButton">取消</button>
    ...
    function confirmDelete(targetElem, path) {
        const escapedFilePath = escapeHtml(path);
        const popoverTitle = '确认删除';
        const popoverContent = `
            <form id="popoverForm">
                <div class="mb-3">
                    确定要删除 <strong>${escapedFilePath}</strong> 吗？
    ...
    function confirmCreateFolder(targetElem, path) {
        const escapedFilePath = escapeHtml(path);
        const popoverTitle = '新建目录';
        const popoverContent = `
            <form id="popoverForm">
                <div class="mb-3">
                    <label for="popoverUserTextInput" class="form-label">目录名称：</label>
